...
public class Coin extends Node implements Node.OnTapListener, MediaPlayer.OnCompletionListener {
    ...
    private Node coni_node;
    private ArFragment arFragment;
    private ModelRenderable coin_renderable;
    private MediaPlayer mediaPlayer; //동영상재생
    private MediaPlayer soundPlayer; //음악재생
    private int sound_play_position = 0; //중지될 위치
    ...
    public void onTap(HitTestResult hitTestResult, MotionEvent motionEvent) {
        ...
        if(coinCheck) {
            ...
            if (coni_node != null) {
                this.removeChild(coni_node);
                coni_node = null;

            }

            if (coin_renderable != null) {
                coin_renderable = null;
            }
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    if(standyPower!=null) {
                        ...
                        if(NavActivity.standy_power!=null){
                            NavActivity.standy_power.setStart_time();
                            NavActivity.standy_power.setCalc(true);
                        }

                        //결과 디비에 반영
                        NavActivity.m_reference.child("StandyPower").child("calc").setValue(true);
                        NavActivity.m_reference.child("StandyPower").child("start_time").setValue(System.currentTimeMillis());
                        NavActivity.m_reference.child("operation").setValue(2);
    ...
    private void createconi_node(){

        float videoWidth = mediaPlayer.getVideoWidth();
        float videoHeight = mediaPlayer.getVideoHeight();

        coni_node = new Node();
        coni_node.setLocalScale(
                new Vector3(
                        VIDEO_HEIGHT_METERS * (videoWidth / videoHeight), VIDEO_HEIGHT_METERS, 1.0f));

        coni_node.setParent(this);
        coni_node.setRenderable(coin_renderable);
        coni_node.setOnTapListener(this);
    ...
    public void pauseEvent(){
        ...
        if(soundPlayer != null) {
            sound_play_position = soundPlayer.getCurrentPosition();
            soundPlayer.pause();
        }
    ...
    public void resumeEvent(){
        ...
        if(soundPlayer != null && !soundPlayer.isPlaying()) {
            soundPlayer.start();
            soundPlayer.seekTo(sound_play_position);
        }
    }
